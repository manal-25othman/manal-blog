...
for (const file of files) {
  ...
  if (!/^##\s+.*(لا يصلح|لا يهمّ|لا تحتاج|لا يستحق|لا يكفي|مبالغة|المخاطرة مقبولة).*$/m.test(body)) {
    warn("لا يوجد قسم يحدّد حدود الحلّ — سياسة التحرير تشترطه");
  }
  ...
  if (words < 350) fail(`المقال قصير (${words} كلمة)`);
}
